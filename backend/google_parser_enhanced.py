...
def extract_google_items_standard(text_content: str, base_fields: dict) -> List[Dict[str, Any]]:
    ...
    # Clean lines from zero-width characters
    ...
    # Also look for specific large amounts we know exist
    known_amounts = ['479,843.67', '319,902.96', '479,901.86', '319,950.09']
    ...
    while i < len(lines):
        ...
        if match or any(amt in line for amt in known_amounts):
            try:
                # Extract amount
                if match:
                    amount = float(match.group(1).replace(',', ''))
                else:
                    # Extract from known amounts
                ...
                for j in range(i-1, max(0, i-10), -1):
                    ...
                    if len(check_line) > 10 and not amount_pattern.match(check_line):
                        description = check_line
                        break
                
                # If no description found, check forward
                if not description:
                ...
                # Skip totals and very small amounts
                if 100 < abs(amount) < 900000:  # Between 100 and 900k
                    if not description:
