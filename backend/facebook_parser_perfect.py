#!/usr/bin/env python3
"""
Facebook parser to achieve 100% accuracy
"""
...
def extract_facebook_ap(text_content: str, base_fields: dict) -> List[Dict[str, Any]]:
    """Extract Facebook AP items"""
    
    items = []
    lines = text_content.split('\n')
    
    # Debug
    line_count = sum(1 for line in lines if line.strip().isdigit() and line.strip() and 1 <= int(line.strip()) <= 999)
    print(f"  Debug: Found {line_count} line numbers in {len(lines)} total lines")
    
    i = 0
    while i < len(lines):
        line = lines[i].strip()
        
        # Check if this is a line number (standalone number)
        if line.isdigit() and 1 <= int(line) <= 999:
            ...
            # Collect lines until we find amount
            while j < len(lines):
                next_line = lines[j].strip()
                
                # Check for amount (number with decimal)
...
if __name__ == "__main__":
    ...
    print("Testing Facebook Parser - 100% Accuracy Goal")
    print("="*60)
