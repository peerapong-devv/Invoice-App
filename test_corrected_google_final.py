...
from corrected_google_parser_final import parse_google_text

def test_all_google_invoices_corrected():
    """Test all 57 Google invoices with corrected parser that extracts actual invoice total"""
    ...
    report_data = {
        "report_metadata": {
            ...
            "parser_version": "corrected_google_parser_final_actual_invoice_total"
        },
        "summary": {
            "successful_files": 0,
            "error_files": 0,
    ...
    for i, filename in enumerate(google_files, 1):
        ...
        try:
            ...
            # Parse using the corrected Google parser
            ...
            if records:
                ...
                file_result = {
                    ...
                    "invoice_type": records[0].get('invoice_type') if records else None,
                    "parser_used": "corrected_final"
                }
                ...
                print(f"  SUCCESS: {len(records)} records, Total: {file_amount:,.2f} THB, Type: {invoice_type}")
                
                # Show key details for first record
                if records:
                    first_record = records[0]
                    if invoice_type == "Credit Note":
                        print(f"     Credit Note - Original Invoice: {first_record.get('original_invoice', 'N/A')}")
                    elif invoice_type == "AP":
                        project_id = first_record.get('project_id', 'None')
                        campaign_id = first_record.get('campaign_id', 'None')
                        print(f"     AP - Project ID: {project_id}, Campaign ID: {campaign_id}")
    ...
    print(f"\n" + "="*80)
    print("CORRECTED GOOGLE INVOICE PROCESSING REPORT")
    print("="*80)
    ...
    print(f"\nTarget Total: 313,085.90 THB")
    print(f"Difference: {report_data['summary']['total_amount_thb'] - 313085.90:,.2f} THB")
...
if __name__ == "__main__":
    test_all_google_invoices_corrected()